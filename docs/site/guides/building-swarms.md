...
uv tool install "swarmkit-runtime[serve,ui]"       # the runtime + `swarmkit` CLI, the server, the portal
swarmkit init                                       # scaffold a workspace through conversation
```

The extras are what make it a full install: `[serve]` (the HTTP server's dependencies) and `[ui]` (the web portal `swarmkit serve` hosts; without it serve runs headless, API only). `[postgres]` adds the Postgres backend. `uv tool install swarmkit-runtime` alone is the CLI. Re-running `uv tool install` upgrades in place.
