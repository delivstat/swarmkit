...
| `sandbox` | no | Isolation tier. Absent ⇒ native git-worktree (default). `kind: container` runs the harness in docker/podman with an `image` or a `build` (`base` + `install`, or a `dockerfile` path relative to the workspace, or `dockerfile_inline` to keep the adapter one file), `mounts`, a `network` policy (`deny` default \| `allowlist` + `allow: [hosts]`), and `resources` (`cpus`, `memory` e.g. `2g`, `pids`). |
| `launch.mcp_tool_name` | no | How this harness spells a governed gateway tool in its own tool grant: `{tool}` is the gateway's flat `<server>__<tool>` name, `{gateway}` the MCP server SwarmKit registers (`swarmkit`). Declared per adapter because the mangling is harness-native — Claude Code's is `mcp__{gateway}__{tool}`. A grant written in any other spelling matches nothing. |
| `grant` | no | How an approved capability set is replayed on a grant-expanding resume under `relay`: `arg` (e.g. `[--allowedTools, "{grant.capabilities}"]`) and `separator` (default a space) joining the approved capabilities into `{grant.capabilities}`. |
