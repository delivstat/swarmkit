...
The portal's **Jobs** view shows live queue health from `GET /queue/stats` — backlog depth,
oldest-unclaimed age, queue-wait and execution p95, and depth by topology (queue-observability.md):

![The portal Jobs view with the queue-health strip](img/portal-queue-strip.png)
...
When both ends are SwarmKit (`a2a-federation.md`), a call returns the callee's run id, token/cost
usage and an observability pointer, and the caller forwards its remaining budget. The portal's
**Audit** view turns the returned run id into a link — an `a2a.remote_usage` event opens the callee
instance's own job detail, so a federated call no longer dead-ends at "called a remote agent":

![The audit log with an a2a.remote_usage deep-link to the callee instance's run](img/portal-a2a-deeplink.png)
