...
def _build_system_prompt(
    ...
    if planning_tools:
        ...
        if _phase1:
            parts.append(
                "\nPHASE 1 — RESEARCH & SOLUTION DESIGN:\n"
                "You are in the research phase. Gather information, then "
                "reason about the solution.\n\n"
                "1. Call `create-task-plan` with research tasks for your "
                "workers. Only research tasks — no document writing.\n"
                "2. After workers complete, call `read-task-result` to "
                "read the FULL findings from each worker.\n"
                "3. REASON about the solution: based on ALL findings, "
                "decide what specific changes are needed, where, and why. "
                "Cross-reference code analysis with CDT configs with Jira "
                "requirements to form a coherent design.\n"
                "4. Call `create-scope` with:\n"
                "   - requirements and constraints from the research\n"
                "   - solution_approach: YOUR DESIGN — each component "
                "change with rationale citing specific findings. This is "
                "the core of the document. Be specific: name files, "
                "services, pipelines, and explain the reasoning.\n"
                "   - open_questions: anything research couldn't resolve\n\n"
                "The solution_approach you write here becomes the "
                "authoritative design for the final document. The "
                "synthesizer will structure it into the template — it "
                "will NOT reason about the solution itself.\n\n"
                "Do NOT create diagrams or write documents. "
