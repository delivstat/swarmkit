...
3. Every artifact must have `apiVersion: swael/v1` and the correct `kind`.
...
- **Always include** `storage: { checkpoints: { backend: sqlite, path: ./.swael/state } }` \
...
mcp_servers:
  - id: weather-api
    transport: stdio
    command: ["python", ".swael/mcp-servers/weather-api/server.py"]
    env:
