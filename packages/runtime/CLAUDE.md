...
6. **Topologies stay portable, open data.** Swarms are interpretable YAML/JSON, never generated code — the no-lock-in guarantee is the openness of the artifacts plus the OSS runtime. (`swarmkit eject` code-export was dropped as the runtime outgrew standalone codegen.)
