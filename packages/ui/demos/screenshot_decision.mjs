...
await p.goto("http://127.0.0.1:3011/runs?run=run-42&stage=greeter", { waitUntil: "networkidle" });
...
await row.locator("textarea").fill("The retry loop has no backoff. Add exponential backoff before this ships.");
...
console.log("panel:", (await p.locator("aside, body").first().innerText()).replace(/\s+/g," ").slice(0, 400));
await b.close();
