...
**Status:** shipping. `swarmkit-runtime` is at **1.247.0** on PyPI (1.0.0 was 2026-04-26); `swarmkit-schema`, `swarmkit-webui` and `swarmkit-control-plane` version independently. Phases 1–5 of `design/IMPLEMENTATION-PLAN.md` are complete, ~4,000 tests run in CI (2,765 runtime, 379 schema, 239 control plane, 610 TypeScript), and the design doc is now a record of *why*, not a plan of *what next* — where the two disagree, the code is what shipped.
...
2. **Skills are the only *capability* extension primitive.** Five backings, one noun: `mcp_tool`, `llm_prompt`, `composed`, `command` (a local binary from a workspace `command_packs` entry — `design/details/command-packs.md`) and `agent` (another agent — a topology here as a child run, or a remote A2A agent through its card — `design/details/a2a-interop.md`). When tempted to add a parallel *capability* mechanism, ask how it could be a skill category, a composed skill or a command pack instead. MCP was never the paradigm; skills were. *Node execution* is a separate provider seam — the `executor` abstraction (`design/details/executor-abstraction.md`), where `model` and `harness` are kinds — parallel to `ModelProvider`/`GovernanceProvider`, not a capability primitive. (A session-holding, diff-producing harness is an executor; a stateless answer-and-return consult is still a skill.)
...
9. **Every interface goes through the same services.** CLI, `POST /run`, chat, both MCP front doors, A2A, webhooks, triggers and workers start work and mutate state through `JobService` / `ArtifactService` / `WorkspaceConfigService` / `CredentialService` / `StorageService` — never by reaching past them into the runtime. A second path is a second set of rules to keep in step, and it drifts in silence: the only symptom is a run behaving differently depending on which door it came through (`/mcp/` had no capacity gate for months; chat ran the base version of a canaried topology). A call site that is genuinely different is marked `# noqa: service-layer` **with the reason**, and `tests/test_service_layer_boundary.py` enforces it. Before folding a bespoke path into the shared one, read it for what it knows — sometimes the bespoke path is the shared one *plus* lessons. See [`docs/notes/service-layer-discipline.md`](docs/notes/service-layer-discipline.md).
