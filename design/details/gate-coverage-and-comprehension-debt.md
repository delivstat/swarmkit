...
- **Slice 7 — `slice_budget` (first schema change) ✅.** Optional `slice_budget` (`max_diff_lines` / `max_files`) on the stage. Full schema discipline (canonical + bundled copy + valid/invalid fixtures + pydantic/TS codegen) **+ the composer form** — the hand-built `StageInspector` renders/edits it (schema→UI step 6, verified: `readStages`/`setSliceBudget` round-trip + inspector inputs). Deterministic measurement (`slice_budget.py`, reusing the diff parser) surfaced as `swarmkit slice-check` (exit 1 over budget, CI-gatable); the SDLC `build` stage declares a budget. *Auto-routing an over-budget change to the funnel `review` layer is the runtime-enforcement follow-on* (same status as cited-change's funnel wiring).
- **Slice 8 — plan-first (schema + gate) ✅.** Optional `objective` / `acceptance` on the stage (the second schema change — full discipline: canonical + bundled + valid/invalid fixtures + pydantic/TS codegen). **Gate coverage flags a stage with no objective** (`swarmkit gates` shows a "no objective" note + `objective` in the payload). A reference `plan-objective` decision skill judges a present objective's *clarity* (the highest-leverage upstream edge). Composer: `StageInspector` gains objective + acceptance inputs (schema→UI, verified via `readStages`/`setObjective`/`setAcceptance` round-trip); the SDLC `build` stage declares both. *Refusing to advance a stage with no clear objective is the runtime-enforcement follow-on* (the decision skill is the reference gate).
