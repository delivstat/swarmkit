...
| 5 | M18 | Workflow archetypes + interop | 🔶 | Sequential/Parallel/Loop archetypes; **A2A server shipped (1.221.0: card + task API + streaming + gate rule)**, `agent` skill next (`a2a-interop.md`); Agent Spec importer (`agent-spec-import.md`) proposed |
...
- [ ] A2A agent / sub-swarm / another instance's swarm modelled as an **`agent` skill**
      (`a2a-interop.md` slice 3: local child job, then remote card; `on_input_required`).
