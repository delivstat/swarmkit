...
| 5 | M18 | Workflow archetypes + interop | 🔶 | Sequential/Parallel/Loop archetypes; **A2A server (1.221.0) and `agent` skill (1.222.0: local child run + remote A2A client, `on_unanswerable`) shipped**; `pack:workspace` (1.223.0), portal (1.224.0), harness gateway (1.225.0); fleet card listing next (`a2a-interop.md`); Agent Spec importer (`agent-spec-import.md`) proposed |
...
- **Schema identifiers.** JSON Schemas use domain-independent URN `$id`s (`urn:swarmkit:schema:v1:*`); validation is bundled and local, so no domain needs to host them. Resolved — not blocking.
...
- [x] Schema identifiers — URN `$id`s (`urn:swarmkit:schema:v1:*`), domain-independent; validation is local (bundled schemas), so no hosting is required.
...
- [x] A2A **server** — Agent Card + task API as a transport onto jobs, streaming, the
...
- [x] A2A agent / sub-swarm / another instance's swarm as an **`agent` skill** — local child
      ...
      1.222.0); `pack:workspace` (1.223.0); portal "Add remote agent" (1.224.0); harness gateway (1.225.0). Left: fleet listing of cards.
