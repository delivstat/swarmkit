...
| 5 | M18 | Workflow archetypes + interop | 🔶 | Sequential/Parallel/Loop archetypes; **A2A server (1.221.0) and `agent` skill (1.222.0: local child run + remote A2A client, `on_unanswerable`) shipped**; `pack:workspace` (1.223.0), portal (1.224.0); harness gateway next (`a2a-interop.md`); Agent Spec importer (`agent-spec-import.md`) proposed |
...
- [x] A2A agent / sub-swarm / another instance's swarm as an **`agent` skill** — local child
      ...
      1.222.0); `pack:workspace` (1.223.0); portal "Add remote agent" (1.224.0). Left: the harness gateway.
