---
title: Implementation Plan — Swael v1.0
...
# Implementation Plan — Swael v1.0

**Source of truth:** `design/Swael-Design-v0.6.md` (§20.1 lists the Phase 1 scope). This plan decomposes that scope into milestones and features, each of which becomes one or more PRs under the [feature delivery workflow](../CLAUDE.md#feature-delivery-workflow--mandatory).
...
| 1 ✅ | Topology loading & resolution | `swael validate path/to/topology.yaml` prints a resolved tree; archetype and skill refs resolved. |
...
| 3 ✅ | LangGraph compiler — capability + coordination | `swael run hello-topology.yaml` executes a two-agent swarm and prints the final state. |
| 3.5 ✅ | Conversational authoring (v1) | `swael init` in an empty dir produces a working workspace through conversation. |
...
| 7 | Reference: Skill Authoring Swarm | `swael author skill` launches a conversational chat that produces a tested, published skill. |
| 8 | Reference: Workspace Authoring Swarm | `swael init` in an empty dir produces a working workspace through conversation. |
| 9 | Eject + HTTP server + scheduled mode | `swael eject` produces runnable LangGraph code; `swael serve` accepts HTTP triggers. |
...
- [x] `feat(schema): pydantic model codegen from JSON Schema` — PR #12. Generated models live in `swael_schema.models`.
- [x] `feat(schema): typescript type codegen from JSON Schema` — PR #13. Generated types re-exported from `@swael/schema`.
...
- [x] `feat(cli): swael validate <path>` + `human-readable validate errors` — PR #23 (tasks #31 + #23). `--json`/`--tree`/`--quiet`/`--color` flags; errors carry file, JSON pointer, rule id, suggestion.
...
- [x] `feat(cli): swael knowledge-pack` — **task #24**, this PR. Bundles the Swael corpus + optional workspace + validation state into a paste-ready markdown prompt (~350 KB). Auto-discovers `design/details/*.md` and `docs/notes/*.md` so new notes land without editing the CLI.
...
- [x] `feat(governance): MockGovernanceProvider` — deterministic, assertable, used in all unit tests and the `swael run` CLI path.
...
- [ ] `feat(cli): wire governance provider selection in swael run` — CLI currently hardcodes `MockGovernanceProvider()`; should read `workspace.yaml` `governance:` block and instantiate `AGTGovernanceProvider.from_config()` when `provider: agt`.

**Exit demo (partially verified):** `test_agt_exit_demo_deny_and_audit` — worker requests `deploy:prod` scope it doesn't have; AGT policy denies; FlightRecorder audit records the denial with tamper-evident hash chain. Full end-to-end through `swael run` with AGT governance awaits CLI wiring.
...
**Status: ✅ COMPLETE.** All built-in providers shipped and are exercised by `swael run` and `swael init`. Provider registry with env-var auto-discovery works. `SWARMKIT_PROVIDER` / `SWARMKIT_MODEL` env-var overrides work.
...
**Exit demo (verified):** `swael run` dispatches to whichever provider has credentials in the environment. `SWARMKIT_PROVIDER=google SWARMKIT_MODEL=gemini-2.5-flash` overrides per-agent provider declarations. Missing SDKs / creds are skipped at registration time.
...
**Status: ✅ COMPLETE** (PRs #35 and related fixes). `swael run` executes two-agent topologies end-to-end. Delegation, skill dispatch, and synthesis all work.
...
- [x] `feat(runtime): SQLite checkpointer wiring` — per-topology checkpoint file under `.swael/state/`.
- [x] `feat(cli): swael run <topology>` — one-shot execution (§14.1). PR #35.

**Exit demo (verified):** `swael run examples/hello-swarm/workspace hello` — root supervisor delegates to greeter worker, worker executes, root synthesises final output. `just demo-run` runs the full flow.
...
**Status: ✅ COMPLETE** (PR #37, hardened by PRs #36, #48, and related fixes). `swael init` and `swael author` both work. `swael author mcp-server` also shipped as part of M5.
...
- [x] `feat(cli): swael init` — interactive workspace creation from scratch. PR #37.
- [x] `feat(cli): swael author topology/skill/archetype` — interactive artifact authoring in an existing workspace. PR #37. Multiple hardening fixes: YAML extraction from code blocks (PR #48), graceful fallback when model skips tool calling, stronger prompts for detailed archetypes.
- [x] `feat(cli): swael author mcp-server` — conversational MCP server authoring. Landed as part of M5 work.

**Exit demo (verified):** `swael init` — user answers questions, gets a working workspace with topology + archetypes + skills. `swael validate` passes. `swael run` produces output.
...
- [x] `feat(runtime): inline HITL + review/gaps CLI commands` — `swael review`, `swael gaps`. PR #42.
...
- [x] `feat(cli): swael author mcp-server` — conversational MCP server authoring. Landed alongside M5 MCP work.
...
- [ ] `design/details/knowledge-mcp-server.md` — **task #25.** Spec an MCP server that exposes the Swael corpus live. Implementation can fold into this milestone or slip to M6.
...
- [ ] `feat(cli): swael author skill [name]` — launches the topology in chat mode.
...
**Exit demo:** `swael author skill` — live conversation produces a new skill YAML, runs its test case against a real MCP server, and publishes to the workspace on user approval. Design doc's 10-minute first-extension promise (§3.4) validated.

## Milestone 8 — Reference topology: Workspace Authoring Swarm

**Goal:** `swael init` in an empty directory produces a working workspace through conversation. This is the v1.0 non-developer on-ramp.
...
- [ ] `feat(cli): swael init` — launches in empty directory.
...
**Exit demo:** `mkdir my-swarm && cd my-swarm && swael init` — user answers a few questions; a runnable workspace exists at the end; `swael run` executes it. Design doc's 15-minute first-run promise (§3.4) validated. A first-time analyst (no prior Swael context) completes this without reading the design doc.
...
- [ ] `feat(runtime): swael eject <topology>` — writes `./generated/` with standalone LangGraph code.
- [ ] `feat(runtime): FastAPI HTTP server for persistent mode`.
- [ ] `feat(runtime): scheduler (cron, webhook, file_watch)`.
- [ ] `test(runtime): ejected code runs without swael installed` — CI step.

**Exit demo:** eject the code-review swarm, install only its `requirements.txt` in a fresh venv, run it. No Swael dependency. Output matches the in-framework run.
...
**Exit demo:** public launch post on GitHub Discussions + Discord announcement; a first user can `pip install swael`, `swael init`, and have a working swarm in <15 min.
