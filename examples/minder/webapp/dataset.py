...
_TRAIN_PY = """\
# Off-box training for a Minder custom detector — run on a GPU (Colab / your machine).
# pip install ultralytics, then: python train.py
from ultralytics import YOLO

YOLO("yolov8n.pt").train(data="data.yaml", epochs=100, imgsz=640, batch=16)
# -> runs/detect/train/weights/best.pt  — import this back into Minder.
"""


def export(name: str) -> dict:
    """Write data.yaml + train.py + README and zip the YOLO dataset for off-box
    training. Returns the zip path."""
    meta = _meta(name)
    if not meta:
        return {"error": "unknown dataset"}
    d = _dir(name)
    ...
    (d / "README.txt").write_text(
        f"Minder custom detector dataset: {meta['name']} (class: {cls})\n\n"
        "1. Upload this folder to Colab or a machine with a GPU.\n"
        "2. pip install ultralytics\n"
        "3. python train.py\n"
        "4. Import runs/detect/train/weights/best.pt back into Minder.\n"
    )
    zip_path = d.parent / f"{meta['slug']}_dataset.zip"
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        for f in ("data.yaml", "train.py", "README.txt"):
            zf.write(d / f, f)
        for sub in ("images", "labels"):
            for p in sorted((d / sub).glob("*")):
                zf.write(p, f"{sub}/{p.name}")
    n_lab = len(list((d / "labels").glob("*.txt")))
    return {
        "zip": str(zip_path),
